		...
		icRender: function(data, options) {
			...
			if (options) {
				if (typeof options == 'string') {
					...
					// options.view override defaultView and set('view','xxx') method
					format = options.format || format;
					view = options.view ? this.set('view', options.view) : view;
			...
			if (format == 'html') {
				if (view) {
					// render view with async model: call icRun
					this.icRun(function(req, res, next) {
