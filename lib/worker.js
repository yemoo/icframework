...
function initFrameConfig() {
    ...
    // shortcut
    exports.require = exports.utils.frame.require;
    ...
    if (!fs.existsSync(config.configpath)) {
        console.error('configpath "' + config.configpath + '" is not exist!');
        killProcess(true);
    }
    // 验证 controller目录 是否存在，必须存在
    if (!fs.existsSync(config.ctrlpath)) {
        console.error('controller path "' + config.ctrlpath + '" is not exist!');
        killProcess(true);
    }
    // 验证 view目录 是否存在
    if (config.view && config.view.path && !fs.existsSync(config.view.path)) {
        logger.warn('view path "' + config.view.path + '" is not exist!');
        killProcess(true);
    }

    // check node version
    if (!semver.satisfies(process.version, minNodeVer)) {
        console.error('Please update your version of the node to ' + minNodeVer + ' or above!\n');
        killProcess(true);
    }
...
function initFrameEvent() {
    ...
    process.on('uncaughtException', function(err) {
        ...
        exceptionLogger.info('trying exit the worker process ' + process.pid);
...
function initOthers() {
    var memwatch = require('memwatch'),
        maxleaktime = 5, // 内存泄露警告5次，自动退出
        leaktime = 0;

    memwatch.on('leak', function(info) {
        exceptionLogger.warn(info);
        if (leaktime++ > maxleaktime) {
            exceptionLogger.info('trying exit the worker process ' + process.pid);
            process.kill(process.pid, 'SIGTERM');
        }
    });
...
function startServer() {
    var APP = exports.APP = require('./application'),
        app = exports.app = exports.APP.app,
        server;
...
function stopServer() {
    ...
    if (exports.requests <= 0) {
        ...
        process.once('SERVER_FREE', function() {
            killProcess(false);
        });
...
function killProcess(closeMaster) {
