...
var configDir = process.cwd() + '/config',
    ...
    loadConfig = function(ret, name) {
        ...
        // Session
        if(ret.session && typeof ret.session.store == 'function'){
            ret.session.store = ret.session.store(express);
        }
        ...
        if (ret.monitor && typeof ret.monitor == 'object') {
            ...
            Object.keys(ret.monitor).reverse().forEach(function(name) {
                originMonitor = ret.monitor[name];
                if (typeof originMonitor == 'function') {
                    originMonitor = originMonitor(ret);
                }
                if (typeof originMonitor == 'string') {
                    monitor.push(originMonitor);
                } else if (Array.isArray(originMonitor)) {
                    monitor = monitor.concat(originMonitor);
                }
            });
            ret.monitor = monitor;
        }
