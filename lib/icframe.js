...
function createWorker(obj) {
    ...
    worker.on('message', function(obj) {
        if (obj.cmd == 'REQUEST_START') {
            requestTotal++;
            requestProcessing++;
        } else if (obj.cmd == 'REQUEST_END') {
            requestProcessing--;
...
exports.start = function() {
    ...
    process.on('SIGINT', function() {
        logger.console();
        logger.separator();
        logger.console('Start closing workers, Pleast wait a moment(maxTime: ' + closeMaxTimeout + 'ms)......');
        if (requestProcessing <= 0) {
            killAllWorkers();
        } else {
            process.once('SERVER_FREE', killAllWorkers);
            // timeout: 10s
            closeTimer = setTimeout(killAllWorkers, closeMaxTimeout);
        }
    });
