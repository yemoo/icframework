...
function checkConfigPath(_path) {
    _path = path.resolve(_path);
    if (!fs.existsSync(_path)) {
        logger.console('Sorry, configPath: "' + _path + '" is not exist! reset to default configPath', true);
        return false;
    }
    return _path;
}
...
exports.start = function() {
    ...
    program.version(packageInfo.version)
        ...
        .option('-c, --configPath <path>', 'Set config directory, default use ' + process.cwd() + '/config', checkConfigPath)
        .parse(process.argv);

    procNum = program.procNum || procNum;
    configPath = program.configPath || configPath;
