...
// 文件cache：controller文件/url->action方法/filter文件/模版文件, config/plugin不能禁止缓存
...
var DEFAULT_CTRL = 'index',
    ...
    APP = {
        ...
        configApp: function() {
            ...
            if (config.log && config.log.accessOpt) {
                app.use(log4js.connectLogger(log4js.getLogger('access'), config.log.accessOpt));
            }
            ...
            Object.keys(plugins).forEach(function(pname) {
                ...
                if (typeof plugin == 'function') {
                    ...
                    app.use(function(req, res, next) {
                        plugins[pname].apply(this, arguments);
                    });
                }
            });
        ...
        _getInstance: function(fileName) {
            ...
            if (!config.cache || !(fileName in controllerRegistry)) {
                ...
                if (fs.existsSync(filePath)) {
                    Controller = utils.frame.require(filePath);
        ...
        _getCallback: function(urlConfig) {
            var instance = urlConfig.instance,
                ...
                // reload filters every time when cache is disabled
                initFilter = !config.cache && filterUtil.init(),
