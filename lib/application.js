...
var DEFAULT_CTRL = 'index',
    ...
    APP = {
        ...
        configApp: function() {
            ...
            // 静态资源映射
            if (config.static) {
                var configType = utils.frame.getType(config.static);

                function addOneStaticRoute(staticConfig) {
                    var prefix = staticConfig.prefix,
                        staticpath = staticConfig.path;

                    if (!prefix) {
                        prefix = '/';
                    }

                    if (typeof prefix !== 'string' || typeof staticpath !== 'string') {
                        logger.warn('invalid static config item', staticConfig);
                        return false;
                    }

                    staticpath = path.resolve(config.ctrlpath, staticpath);
                    if (staticpath === config.ctrlpath) {
                        logger.warn('static path cannot equal the controller path', staticConfig);
                        return false;
                    }
                ...
                function addStaticRoute(staticConfig) {
                    if (utils.frame.isType(staticConfig.path, 'array')) {
                        // {prefix: '/static', prefix:['/static1','static2']}
                        staticConfig.path.forEach(function(item) {
                            addOneStaticRoute({
                        ...
                        addOneStaticRoute(staticConfig);
                    }
                }
                ...
                if (configType === 'array') {
                    ...
                    addStaticRoute(config.static);
                } else {
                    logger.warn('invalid static config', config.static);
                }
            }
            ...
            app.use(function(req, res, next){
                req._beforeplugintime = Date.now();
                next();
            });
            // plugins
            if (config.cache) {
            ...
            return function(req, res) {
                ...
                if (before) {
                    Object.keys(before).forEach(function(key) {
                        var cb = wrapReqCallback(before[key]);
                        cb.NAME = key;
                        ctrlUtil.run(cb);
                    });
                }
                ...
                function userCallback(req, res, next) {
                    req._usercallbacktime = Date.now();
                ...
                if (after) {
                    Object.keys(after).forEach(function(key) {
                        var cb = wrapReqCallback(after[key]);
                        cb.NAME = key;
                        ctrlUtil.run(cb);
                    });
                }
