...
var APP = {
        ...
        if (!this.viewRegistry[cacheKey]) {
            ...
            if (viewPath) {
                ...
                viewPath.every(function(fname, index, arr) {
                    ...
                    // find as folder,  // always force last section find as file
    ...
    configRouter: function() {
        ...
        Object.keys(routerConfig).forEach(function(url) {
            ...
            urlConfig.mappedUrl = urlConfig.mappedUrl.replace(/(^\/|\/$)/g, '');
            app[urlConfig.method](url, self._getUrlConfig(url, true).callback);
        });
