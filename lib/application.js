...
var DEFAULT_CTRL = 'index',
    ...
    APP = {
        ...
        configApp: function() {
            ...
            app.set('view engine', viewCfg.defaultEngine);
            ...
            // Session config
            if (config.session && config.session.cookie && config.session.cookie.expires) {
        ...
        _getViewPath: function(urlConfig) {
            ...
            if (!(viewpath in this.viewRegistry)) {
                ...
                if (path.extname(originpath)) {
                    ...
                    // auto add ext
                    ext = ('.' != view.defaultEngine[0] ? '.' : '') + view.defaultEngine;
