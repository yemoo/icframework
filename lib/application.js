    ...
    APP = {
        ...
        configApp: function() {
            ...
            Object.keys(viewEngineMap).forEach(function(viewExt) {
                app.engine(viewExt, viewEngineMap[viewExt]);
            });
            ...
            app.render = function(name, options, fn) {
                ...
                timer = setTimeout(function() {
                    clearTimeout(timer);
                    fn(new Error('template "' + name + '" render timeout!'));
                }, config.viewRenderTimeout);

                appRender.apply(app, arguments);
            };
        ...
        _getUrlConfig: function(url) {
            ...
            if (!curConfig || !curConfig['parsed']) {
                ...
                if (success) {
                    ...
                    if (curConfig.view) {
                        curConfig.view = curConfig.view.replace(/(^\/|\/$)/g, '');
                    }
        ...
        _getViewPath: function(urlConfig) {
            var viewpath = urlConfig.view || urlConfig.mappedUrl,
                originpath, filepath, ext;

            if (!(viewpath in this.viewRegistry)) {
                originpath = path.join(config.viewDir, viewpath);
                if (path.extname(originpath)) {
                    filepath = fs.existsSync(originpath) ? originpath : false;
                } else {
                    ext = ('.' != config.viewEngine[0] ? '.' : '') + config.viewEngine;
                    filepath = originpath + ext;

                    if (!fs.existsSync(filepath)) {
                        filepath = originpath + '/index' + ext;
