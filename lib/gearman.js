    ...
    serverDead: function(server) {
        ...
        (function() {
            ...
            client.on('connect', function() {
                ...
                client.close();
                // 重连成功则初始化连接
                logger.info("Server " + server.ip + ':' + server.port + " reconnect success!");
                self.init();
            });

            client.on('error', function(err) {
                if (['ECONNREFUSED', 'EHOSTUNREACH', 'ECONNRESET', 'ETIMEOUT'].indexOf(err.code) > -1) {
                    if (i++ < config.retries) {
                        setTimeout(callee, config.retry);
                    } else {
                        i = 0;
                        // 超过最大尝试次数
                        logger.info("fail " + config.retries + " times, retry connnect server " + server.ip + ':' + server.port + ' after ' + config.reconnect + 'ms');
            ...
            client.connect();
            // 尝试retries次连接
            logger.info("retry(" + i + ") connect server " + server.ip + ':' + server.port);
        })();
    },
    /**
     * 获取一个可用的server
    ...
    createClient: function() {
        ...
        if (self.totalClients < config.maxClients) {
            ...
            client.on('connect', function() {
                ...
                self.totalClients++;
                logger.info("a client on " + server.ip + ':' + server.port + " is connected, totalClients: " + self.totalClients);
                self.processStackJob();
            });
            // gearman服务关闭
            client.on("close", function() {
                self.removeClient(client);
                logger.info("a client on " + server.ip + ':' + server.port + " is disconnected, totalClients: " + self.totalClients);
                // 运行时连接数等于0，尝试创建连接
                if (!self.totalClients && self.running) {
                    self.createClient();
                }
            });
            // 初始化连接时出错
            client.on("error", function(err) {
                logger.error("Server " + server.ip + ':' + server.port + ' ' + err.message);
                // 服务器不能连接
                if (['ECONNREFUSED', 'EHOSTUNREACH', 'ECONNRESET', 'ETIMEOUT'].indexOf(err.code) > -1) {
                    // 标记该client对应的服务器不可用
