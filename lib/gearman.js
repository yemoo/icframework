...
GearmanManager = {
	init: function() {
		...
		if (fs.existsSync(icFrame.config.configPath + '/gearman' + '.js')) {
		...
		this.inited && this.initGearmanClient();
	},
	getServer: function() {
	...
	submitJob: function() {
		...
		} else if ((typeof options == 'object') && (options.fn || options.timeout)) {
			args.pop();

			if (options.fn) {
				job.callback = options.fn;
			} else {
				job.background = true;
			}
			// job level timeout 
